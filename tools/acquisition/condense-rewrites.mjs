...
const map = await batchClassify({
  ...
  buildPrompt: (batch) => [
    ...
    'DIAGNOSING DIFFICULTY: the difficulty of a clue is set by its SINGLE most recognizable fact — the most',
    'obvious path to the answer — and MORE facts only make it EASIER (another path in), never harder. When an',
    'edit makes a clue harder, the cause is that it CUT or OBSCURED the most-obvious link (often a category-label',
    'or identifier lead like "This Norse shape-shifter…" or "the six-time NBA champion…"), NOT that it added',
    'detail. NEVER phrase a lesson as "add corroborating details", "pivot to multiple details", or "stack facts"',
    '— fact count is incidental; the obviousness of the easiest link is the whole lever.',
